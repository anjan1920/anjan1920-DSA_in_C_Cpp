...
void display() {
    ...
    if (head == NULL) {
        ...
        //when temp reach head its stop and we print the head node again to represent the circular link
        printf(" |->%d ", temp->data);
        count++;
...
void delEnd(){
    ...
    if( head == 0){
        ...
        //temp points to last and preNode points to 2nd last node
        preNode ->next = head;
        free(temp);
        tail = preNode;
    }
...
void delPos(){
    ...
    if(head == 0){
        ...
        int pos ;
        printf("Enter postion of deletion : ");
        scanf("%d" ,& pos);
